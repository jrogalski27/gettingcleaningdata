...
```r
MyTidyDatatset[, .N, by = c(names(MyTidyDatatset)[grep("^feat", names(MyTidyDatatset))])]
```
...
```r
f <- file.path("DatasetHumanActivity.txt")
write.table(MyTidyDataset, f, quote = FALSE, sep = "\t", row.names = FALSE)
```
